            ...
            <div>
                ...
                <div className="text-sm sm:text-sm xl:text-base leading-[1.5] text-brand ">
                    {data?.categories?.map((v: any, i: number) => (
                        <span key={i} className="hover:underline transit">
                            {v}{" "}
                        </span>
                    ))}
                </div>
                ...
                <div>
                    <div className="flex flex-wrap gap-4">
                        {data?.tags?.map((v: any, i: number) => (
                            <div
                                key={i}
                                className="border hover:text-brand hover:border-brand transit border-neutral p-[8px_20px] rounded-md text-xs text-neutral sm:text-sm xl:text-base"
                            >
                                {v}
                            </div>
                        ))}
